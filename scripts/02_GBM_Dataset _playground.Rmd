...
grep("^VEGFA$", rownames(GBM.only@assays$RNA@data), value = T)
VEGFA.values <- GBM.only@assays$RNA@data[grep("^VEGFA$", rownames(GBM.only@assays$RNA@data)), ]
VEGFA.expression <- VEGFA.values > 0
table(VEGFA.expression)
GBM.only@meta.data$VEGFAstatus <- VEGFA.expression

grep("^IL1B$", rownames(GBM.only@assays$RNA@data), value = T)
IL1B.values <- GBM.only@assays$RNA@data[grep("^IL1B$", rownames(GBM.only@assays$RNA@data)), ]
IL1B.expression <- IL1B.values > 0
table(IL1B.expression)
GBM.only@meta.data$IL1Bstatus <- IL1B.expression
...
scCustomize::VlnPlot_scCustom(GBM.only, 
                 "IL1B", 
                 pt.size = 0.1,
                 group.by = "VEGFAstatus")




p1 <- scCustomize::VlnPlot_scCustom(GBM.only, 
                 "CA9", 
                 pt.size = 0.1,
                 group.by = "CA9status")


p2 <- scCustomize::VlnPlot_scCustom(GBM.only, 
                 "IL1B", 
                 pt.size = 0.1,
                 group.by = "CA9status")
...
table(GBM.only@meta.data$IL1Bstatus, GBM.only@meta.data$VEGFAstatus)
